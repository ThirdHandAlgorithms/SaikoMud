...
void CharToBin( unsigned char c, TGFString *sOutput ) {
   int i;
   unsigned char c2;
...
void StrToBin( TGFString *sInput, TGFString *sOutput ) {
...
void StrToHex( const TGFString *sInput, TGFString *sOutput ) {
   ...
   unsigned int i;
   for ( i = 0; i < sInput->getLength(); i++ ) {
      CharToHex( sI[i], sOutput );
   }
}


void CharsToAscii85( char sInput[], TGFString *sOutput ) {
   unsigned int i = 0;
   char a, b, c, d, e;
...
void StrToAscii85( TGFString *sInput, TGFString *sOutput ) {
   char *sI = sInput->getValue();
   char ic[4] = {0,0,0,0};
...
void CharsToBase64( char sInput[], TGFString *sOutput ) {
   char a, b, c, d;
...
int getCharIndexInHufftable( const TGFString *sHufftable, char ch ) {
...
unsigned long GFGetFileSize( const TGFString *sFilePath ) {
   ...
   TGFString *sTmpFile = new TGFString( sFilePath );
...
bool GFIsDirectory( const TGFString *sFilePath ) {
      ...
      TGFString *sTmpPath = new TGFString( sFilePath );
...
TGFString *createHybridString( const void *s, bool bWide ) {
   if ( bWide ) {
      return new TGFString( reinterpret_cast<const wchar_t *>(s) );
   } else {
      return new TGFString( reinterpret_cast<const char *>(s) );
   }
}

void GFPrependNativeSlash( TGFString *sPath, bool bWide ) {
   TGFString *a;
   if ( bWide ) {
...
void GFAppendNativeSlash( TGFString *sPath, bool bWide ) {
   TGFString *a;
   if ( bWide ) {
      a = createHybridString( S_WIDE_NATIVE_SLASH, bWide );
   } else {
...
void GFConvertToNativeSlashes( TGFString *sPath, bool bWide ) {
   TGFString *a;
   TGFString *b;
   TGFString *c;
   if ( bWide ) {
...
void GFConvertToURISlashes( TGFString *sPath, bool bWide ) {
   TGFString *a;
   TGFString *b;
   if ( bWide ) {
...
__int64 GFDateStrToTimestamp( TGFString *sDate, bool bTZCorrection, bool bDaylightSavingsCorrection ) {
...
TGFVector *GFsplit( const TGFString *sHaystack, const char *sNeedle, int iMax ) {
   TGFVector *slices = new TGFVector();
   TGFString *slice;
   int iCurrentPos = 0;
   int iPreviousPos = 0;
