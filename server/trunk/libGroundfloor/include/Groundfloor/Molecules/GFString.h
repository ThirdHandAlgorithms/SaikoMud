...
/** TGFString is a class that defines a variable length string
 ...
 */
class TGFString: public TGFFreeable {
private:
   TGFString& operator=(const TGFString* s); // not allowed to make deep copies
protected:
...
public:
   ...
   TGFString( const char *sValue, unsigned int iLength );
   TGFString( const TGFString *sValue );

   /// Initializes the string to sValue using strlen() for length (\\0 terminated)
   TGFString( const char *sValue );       // ANSI
   TGFString( const wchar_t *sValue );    // Wide (Windows UTF-16 / Linux UTF-32)
   ...
   bool match( const TGFString *sNeedle ) const;
   bool startsWith( const TGFString *sNeedle ) const;
   bool endsWith( const TGFString *sNeedle ) const;

   int pos( unsigned int iStart, const TGFString *sNeedle ) const;
   int lastpos( const TGFString *sNeedle ) const;

   /// Appends the given TGFString to the current string, adjusts the length and resizes memory space if necessary.
   bool append( const TGFString *sValue );
   /// Appends the given TGFString of given length to the current string, adjusts the length and resizes memory space if necessary.
   bool append( const TGFString *sValue, unsigned int iLength );
   ...
   /// Copies the given TGFString into itself, freeing the old string data.
   bool setValue( const TGFString *sValue );

   /// Replace substring a with substring b
   bool replace( const TGFString *sSubStringA, const TGFString *sSubStringB );

   /// Remove a piece of the string from start to end;
   bool remove( unsigned int iStartPos, unsigned int iEndPos );

   /// Prepends the given TGFString to the current string, adjusts the length and always remaps the string data.
   bool prepend( const TGFString *sValue );
   /// Prepends the given TGFString of given length to the current string, adjusts the length and always remaps the string data.
   bool prepend( const TGFString *sValue, unsigned int iLength );
   ...
   /** If memory failes to allocate; false is returned.
     ...
     *  in order to avoid memory violations. Please use TGFStringVector for big chunks of data.
     **/
   ...
   /// operators
   TGFString& operator=(const char* s);
   TGFString& operator=(const wchar_t* s);

   TGFString& operator<<(const TGFString *s);
   TGFString& operator<<(const char *s);
   TGFString& operator<<(const wchar_t *s);

   friend std::ostream& operator<<(std::ostream& Ostr, TGFString* s);
   friend std::ostream& operator<<(std::ostream& Ostr, TGFString s);
};
