...
TJRBaseSocket::TJRBaseSocket() : TGFBaseCommunicator() {
   resetVars();
}

TJRBaseSocket::TJRBaseSocket( int iSocketId, const TJRBaseSocket *aServer ) : TGFBaseCommunicator() {
   resetVars();
...
TJRBaseSocket::~TJRBaseSocket() {
	disconnect();
}

void TJRBaseSocket::reinitialize( int iSocketId ) {
	disconnect();
...
bool TJRBaseSocket::loadLocalAddressInfo() {
...
bool TJRBaseSocket::loadRemoteAddressInfo() {
...
bool TJRBaseSocket::isReadyToSend() {
...
bool TJRBaseSocket::send( const TGFString *sData, TGFCommReturnData *errData ) {
	int iStatus = -1;
...
bool TJRBaseSocket::receive( TGFString *sData, TGFCommReturnData *errData ) {
   int iStatus = -1;
   bool bIsOk = true;
...
void TJRBaseSocket::setBlocking( bool bBlocking ) {
	#ifdef GF_OS_LINUX
    	int iFlag = 0;
